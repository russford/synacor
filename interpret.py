        ...
        if (verbose):
            ...
            self.trace.append (
                    "{:>7}: {:<19} r:{} {}".format(
                            "[%d]" % cp,
                            f.__name__.strip("_") + " " +
                            " ".join([str(p) if p < 32768 else "r{}".format(p-32768) for p in a]),
...
with open("challenge.bin", "rb") as f:
    bytes = f.read()
    e.code = [bytes[i*2] + (bytes[i*2+1] << 8) for i in range(len(bytes)//2)]
    if len(e.code) < 32768: e.code.append([0]*(32768-len(e.code)))
